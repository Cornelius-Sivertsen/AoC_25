...
// remove all possible rolls, return number of rolls removed
int remove_rolls(vector<string>& input_data){
  int free_rolls = 0;
  vector<string> copy = input_data;
...
int run_case(vector<string> input_data){
  ...
  int total_removed = 0;
  do{
    removed_at_iter = remove_rolls(input_data);
    total_removed += removed_at_iter;
  } while (removed_at_iter != 0);
